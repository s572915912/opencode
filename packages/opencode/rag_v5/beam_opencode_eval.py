...
# 评分开关（可通过 CLI --no-f1 / --no-judge / --no-keyword / --no-score 关闭）
SCORE_F1 = True
SCORE_JUDGE = True
SCORE_KEYWORD = True
...
def _judge_via_opencode(prompt):
    """通过本地 OpenCode API 发送 judge prompt 并获取回复文本"""
...
def score_llm_judge(question, model_answer, expected_answer):
    """通过本地 OpenCode API 调用 LLM 评估语义正确性，返回 0.0~1.0"""
...
def score_answer(question, model_answer, expected_answer):
    """根据全局开关计算启用的评分指标，返回 dict。
    开关：SCORE_KEYWORD / SCORE_F1 / SCORE_JUDGE"""
    ...
    if judge is not None:
        ...
        weights.append((judge, 0.6))
    if f1 is not None:
        weights.append((f1, 0.25 if judge is not None else 0.5))
    if kw is not None:
        weights.append((kw, 0.15 if judge is not None else 0.5))

    if weights:
        total_w = sum(w for _, w in weights)
        combined = sum(v * w for v, w in weights) / total_w
    else:
        combined = 0.0
...
def _score_one(idx, total, q, answer):
    ...
    scores = {"keyword": None, "token_f1": None, "llm_judge": None, "combined": None} if no_score \
        else score_answer(q["question"], answer, q["expected_answer"])
    result = {
        "type": q["type"],
        "question": q["question"],
    ...
    if no_score:
        ...
        print(f"  {emoji} Combined={c:.2f}  ({' '.join(parts)})")
...
def print_results_table(all_results):
    ...
    enabled = [n for n, v in [("F1", SCORE_F1), ("Judge", SCORE_JUDGE), ("Keyword", SCORE_KEYWORD)] if v]
    scoring_label = ', '.join(enabled) if enabled else 'ALL DISABLED'
...
def main():
    """CLI 入口，支持 --no-f1 / --no-judge / --no-keyword / --no-score 关闭评分。"""
    ...
    parser.add_argument("--no-score", action="store_true", help="关闭全部评分（仅收集模型回答）")
